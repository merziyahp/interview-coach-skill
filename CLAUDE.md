...
## Resource Auto-Discovery Protocol

When a company name is referenced in any command, automatically check for resources before asking the candidate to provide them.
...
### Filename-to-Content Inference

Use the filename to understand what each file contains before reading:

| Filename pattern | Assumed content |
|---|---|
| "resume", "cv" | Candidate resume — use for this application |
| "JD", "job description", "role", "position", company name alone | Job description |
| "LI profile", "linkedin" | Interviewer or company LinkedIn profile |
| "OKR", "job mission", "mission" | Job mission / OKR document — high-value context for prep and cover letters |
| "research", "notes", "company" | Company research notes |
| "email", "comms", "followup", "thank you", "follow up" | Email communications — read for context, don't regenerate what was already sent |
| "cover letter", "coverletter" | Previously generated cover letter |
| Anything else | Read and infer from content |
...
**Candidate workflow:** Create `resources/[Company]/` for each new company. Drop in the JD, any research, LinkedIn profiles, OKR docs. The skill reads everything in the folder automatically.
...
| `coverletter [company]` | Generate a tailored cover letter using storybank and company research |
...
- **`coverletter`**: Pull Resume Analysis, Storybank (including earned secrets), and Interview Loops data from `coaching_state.md` before drafting. JD is required — ask if not available.
...
12. "Write me a cover letter" / "cover letter for [company]" -> `coverletter`
